...
export const Inspector = ({ socket }: { socket: SimulationSocket | undefined }) => {
  ...
  if (!inspected) {
    return (
      ...
      >
        <p className={styles.empty}>{t(locale, 'inspector.empty')}</p>
        {interestingList}
      </aside>
    );
  }
    ...
    >
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: portraitSvg(inspected.avatarSeed, inspected.expression, 120),
        }}
      />
      <div className={styles.identity}>
        ...
        <p>
          ...
          <br />
          {formatActionPhrase(locale, inspected)}
        </p>
        <p>
      ...
      <section className={styles.section}>
        ...
        {TRAIT_KEYS.map((key) => (
          <div className={styles.need} key={key}>
            <span>{t(locale, `inspector.${key}`)}</span>
            <div className={styles.track}>
              <div className={styles.fill} style={{ width: `${inspected.personality[key]}%` }} />
            </div>
